...
text(-600,0,'L. facet refl - phase');
Rle=num2str(Rleft);Phl=num2str(phil);
text(-100,0,Rle);text(+100,0,Phl);
text(-600,-5,'R. facet refl. - phase');
Rri=num2str(Rright);Phr=num2str(phir);
text(-100,-5,Rri);text(+100,-5,Phr);drawnow;
...
if pl==2; 
   ...
   disp('Type   spontoo   then press return key to remove noise and continue;');
   %spontoo is a sequence of programs that turns off noise
   %spontoo runs laser and plots output etc giving 'idealised' output etc.
   keyboard; 
   disp('Do you wish to continue? '); 
