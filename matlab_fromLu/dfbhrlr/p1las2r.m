...
disp(' ');
disp('What magnitude of index coupling length product ?');
kLind=input('input (0.5<kL<2(default 1) :  ');
if isempty(kLind); kLind=1;end;
disp(' ');
kL=kLind;
disp(' ');
disp('What magnitude of gain coupling length product ?');
kLgain=input('input magnitude (0<kLgain<2)(default 0) :  ');
if isempty(kLgain); kLgain=0;end;
if kLgain>0;
   disp('Do you wish to have in phase or anti-phase gain coupling:');
	ap=input('type 1 for inphase and -1 for anti-phase (default 1) : ');
	if isempty(ap); ap=1;end;
kL=kL+j*kLgain*ap;		% average complex value kappa L product;
end;
% Left facet parameter input section;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Rleft=input('Enter left facet power reflectivity (default 0.9)');
   if isempty(Rleft);
      Rleft = 0.9;
   end;
...
aH= 4;			 % Henry's linewidth factor -positive number here
