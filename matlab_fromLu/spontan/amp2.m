clear all;
%olortog  % ensures black on white background as default;

disp('DEMONSTRATES LIMITING NOISE TO SIGNAL RATIO AT HIGH GAINS'); 
disp('AS LENGTH OF AMPLIFIER IS INCREASED - ZERO AMPLIFIER LOSS.');
disp('Third demonstration introduces model amplifier'); 
disp('with total quantum inversion'); 
disp('Gain is now introduced by using equation:-');
disp(' dF/dz + dF/(vg t) = g F + Is ');  
disp('Here stimulated field gain g is proportional to P N product'); 
disp('but no stimulated absorption because of maximum inversion');
disp('Random spontaneous emission is also proportional to P N:-');
disp('|Is|^2 = B*PN while g=A*PN ; Normalised default value PN=1.');
...
figure;f0=gcf;  
for n=1:4						%O
L=2^(n-2); % runs through the various lengths, 
Lstr=num2str(L); % for printing length later

PN=1; % Default value of PN product to fix spontaneous emission and gain
A=2;B=1;% default values of normalised gain and spontaneous emission parameters
g=A*PN; BPN=B*PN;
...
ylabel('dB');
text(-.45,64,'Coherent+Spont. Power v.freq','fontsize',9);
...
text(-.5,13,'Laser Norm. Length');
text(1,11.5,Lstr);
text(-.5,8,'Amplitude Gain =')
text(0,7, 'exp(2 x Norm Length)');
