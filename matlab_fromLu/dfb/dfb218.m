...
disp(' However this now has two lambda/8 phase shifts away from centre');
...
% phase shift at two points Nr1 and Nr2 see p2comp program;
costo(Nr1)=exp(j*pi/4)*costo(Nr1); %phase shift of lambda/8 at two points;
sinto(Nr1)=exp(j*pi/4)*sinto(Nr1); %phase shift of lambda/8 at two points;
costo(Nr2)=exp(j*pi/4)*costo(Nr2); %phase shift of lambda/8 at two points;
sinto(Nr2)=exp(j*pi/4)*sinto(Nr2); %phase shift of lambda/8 at two points;
mmm=ones(N,1);
mmm(Nr1)=0; mmm(Nr2)=0;  
% ensures later that no gain or extra phase shifts at these points;
