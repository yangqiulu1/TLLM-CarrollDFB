...
kappaL=2;% 1
gL=0.5; %1 %'dfbamp'里是为了归一化取得gL=1/kappaL么？
deltaL=4; %'dfbamp'说是归一化失谐【？】【看DFB耦合模理论这里是β-β0，的确叫失谐，是光栅引起的】
%m=16; %16个光栅周期，选用材料n1=3.2515;n2=3.456，按m=round(kappaLg/(2*r))计算，【似乎本程序用不到，注意和空间步长对比就好】
N=31; NF=1:1:N+1; %空间步数N；场点数NF
repeat=15*N; % number of repeats of field calculations【?不记得为什么要重复计算场了】
%【看完整个程序觉得，是要让放大器行波达到稳定，这就是运行的t，所以计算15遍，从实时图看，不稳定的话还可以再多算几遍】
kappas=kappaL/N; %每个空间步长上的耦合【！】,s为空间步长，s=L/N

% 原作者吧mL,耦合矩阵sint/cost[对应sin_T/cos_T]写成相同元素的矩阵了，这里都当作常数处理
...
for rep=1:1:repeat
    ff(1) = 1;
    ffn(1) = 1
    ffn(2) = mL*cos_T*ff(1) + j*mL*sin_T*fr(2);
    fr(N+1) = 0;
    frn(N+1) = 0;
    frn(N) = mL*cos_T*fr(N+1) + j*mL*sin_T*ff(N);
    for n=2:N
